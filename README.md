...
1. 在控制台輸入要顯示的文字。
2. 拖曳螢幕上的高亮文字到需要的位置。
3. 使用控制台調整字體大小、透明度、底色與文字色。
4. 按 `Esc` 或關閉控制台結束程式。

## 啟動參數

也可以在啟動時指定文字、大小、顏色、透明度與位置：

```powershell
python screen_highlight_text.py --text "重要提醒" --font-size 48 --highlight-color "#fff176" --text-color "#111111" --opacity 0.85 --x 200 --y 150
```
